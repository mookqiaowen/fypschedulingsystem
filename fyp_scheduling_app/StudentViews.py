...
from fyp_scheduling_app.models import Students, Subjects, Courses, Attendance, AttendanceReport, CustomUser, \
    ScheduleReportStudent, FeedbackStudent, SessionYearModel
...
def student_apply_schedule(request):
    staff_obj = Students.objects.get(admin=request.user.id)
    schedule_data=ScheduleReportStudent.objects.filter(student_id=staff_obj)
    return render(request,"student_template/student_apply_schedule.html",{"schedule_data":schedule_data})

def student_apply_schedule_save(request):
    if request.method!="POST":
        ...
        student_obj=Students.objects.get(admin=request.user.id)
        try:
            schedule_report=ScheduleReportStudent(student_id=student_obj,schedule_date=schedule_date,schedule_message=schedule_msg,schedule_status=0)
            schedule_report.save()
            messages.success(request, "Successfully Applied for Schedule")
            return HttpResponseRedirect(reverse("student_apply_schedule"))
        except:
            messages.error(request, "Failed To Apply for Schedule")
            return HttpResponseRedirect(reverse("student_apply_schedule"))
